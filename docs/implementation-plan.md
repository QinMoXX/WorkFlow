...
- `[~]` 节点运行状态展示：已有状态点和错误/路径文本，缺少真实缩略图预览。
- `[ ]` 图片输入的文件选择器和导入资源管理。
- `[ ]` 图片结果右键菜单。
...
下一步优先做阶段二。原因是当前项目已经具备画布、配置和执行骨架，但 AI 节点仍返回模拟路径；真实 API 接入完成后，整个 MVP 才能形成可验证的图片生成闭环。

建议顺序：

1. 新增 Rust HTTP 客户端依赖和 OpenAI-compatible image adapter。
2. 先实现文生图真实调用。
3. 下载返回图片 URL 到本地生成目录。
4. 再处理图生图的本地图片输入与供应商兼容问题。
5. 最后补图片预览和右键菜单。
