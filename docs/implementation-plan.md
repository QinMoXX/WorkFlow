...
- `[~]` 图片输入的文件选择器和导入资源管理：已支持剪切板图片导入到 `appData/assets/imported/`，文件选择器未实现。
- `[ ]` 图片结果右键菜单。
...
- `[~]` 保存图片、复制图片、打开文件所在目录 commands：输出节点已支持按 `saveDirectory` 自动复制上游图片，右键操作 commands 未实现。
...
- `[~]` 图片输入节点支持本地图片导入：已支持剪切板粘贴图片，文件选择器未实现。
- `[x]` 导入图片复制到 `appData/assets/imported/`。
- `[ ]` 生成缩略图并写入 `thumbnailPath`。
- `[~]` 节点卡片展示图片预览，而不只是路径文本；当前直接展示本地图片，尚未生成独立缩略图文件。
...
下一步优先推进阶段三和阶段四。原因是真实图片 API 和节点内图片预览已经形成基本闭环，但本地图片导入、右键结果操作和长任务实时状态仍缺失。

建议顺序：

1. 实现图片输入节点的本地文件选择、导入复制和缩略图生成。
2. 增加图片结果右键菜单，以及保存、复制、打开所在目录、重新运行操作。
3. 后端通过 Tauri event 推送逐节点运行状态。
4. 前端订阅运行事件，运行长任务时实时更新节点状态和日志。
