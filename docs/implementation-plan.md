...
- `[~]` 画布高级编辑：React Flow 已支持基础多选与批量移动能力，但框选体验、快捷键、撤销重做、节点复制粘贴、自动布局、节点分组、缩放定位仍需产品化实现。
...
- `[x]` 图片结果右键菜单。
- `[ ]` 框选、多选批量移动、快捷键、撤销重做、节点复制粘贴、画布自动布局、节点分组、缩放定位。
...
- `[ ]` 快捷键：
  ...
  - 运行当前节点或运行全部
  - 适配视图
- `[ ]` 撤销重做历史，覆盖节点新增、删除、移动、连线、参数编辑。
- `[ ]` 节点复制粘贴，复制节点参数、相对位置和可复制的内部连线，并生成新的节点 ID。
- `[ ]` 画布自动布局，优先按依赖方向从左到右排布。
- `[ ]` 节点分组，第一版只作为视觉组织和批量移动，不改变执行语义。
- `[ ]` 缩放定位：适配全部节点、定位选中节点、回到默认视图。
...
- `[x]` 第一版只支持 1 个工作流。
- `[x]` 第一版使用手动保存，不做自动保存。
- `[ ]` 为多工作流项目预留数据目录方案：`workflows/{projectId}/workflow.json` 与项目内 assets。
- `[ ]` 增加项目元信息设计：项目 ID、名称、创建时间、更新时间、最近打开时间。
- `[ ]` 后续独立 API route 后端设计：
  - 由后端保存和管理 API Key。
  - 前端工具只调用后端 API route。
  - 后端负责供应商协议转换、鉴权、审计、限流和错误归一化。
- `[ ]` 当前本地明文 API Key 风险在 README 中明确说明。
- `[ ]` 检查 `src-tauri/capabilities/default.json`，只开放必要权限。
- `[ ]` 收紧 `src-tauri/tauri.conf.json` 中 asset protocol scope。
...
下一步优先推进阶段四和阶段五。原因是真实图片 API、节点内图片预览、本地图片导入和结果右键操作已经形成基本闭环，但长任务实时状态和画布高频编辑效率仍缺失。
